...
summarize_trips <- function(track,
  ...
  track %>%
    ...
    summarise(
      ...
      n_fixes = n(),
      max_dist_to_colony_m = if (distance_col %in% names(cur_data())) max(.data[[distance_col]], na.rm = TRUE) else NA_real_,
      prop_day = if ("diel_period" %in% names(cur_data())) mean(diel_period == "day", na.rm = TRUE) else NA_real_,
      prop_night = if ("diel_period" %in% names(cur_data())) mean(diel_period == "night", na.rm = TRUE) else NA_real_,
      prop_commuting = if (phase_col %in% names(cur_data())) mean(.data[[phase_col]] == "commuting", na.rm = TRUE) else NA_real_,
      prop_foraging = if (phase_col %in% names(cur_data())) mean(.data[[phase_col]] == "foraging", na.rm = TRUE) else NA_real_,
      .groups = "drop"
    )
}
